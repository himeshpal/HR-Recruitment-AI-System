...
5. **Keep the pipeline deterministic.** Use plain Python pipelines as a fixed sequence (Job → Resumes → Match → Panel → Interview → Recommendation), with agents inside steps; do not add LangGraph (drive the Live Agent Graph from real LLM-call events instead), not free-form autonomous agents. A human recruiter makes the final decision.
